...
trait Prompter {
  ...
  protected def confirmScriptsInstalledPretext(numScripts: Int): String =
    s"""You are about to install ${if (numScripts == 1) "a file" else s"$numScripts files"} with embedded Lua scripts.""" +
    f""" Lua scripts contain code that extends the functionality of the game (e.g. advisor messages, automata generators and rewards).%n%n""" +
...
class CliPrompter(val logger: CliLogger, autoYes: Boolean) extends Prompter {
  ...
  def confirmScriptsInstalled(scriptsInstalled: Seq[Sc4pac.StageResult.LuaInstalled]): Task[Boolean] = {
    if (scriptsInstalled.isEmpty) ZIO.succeed(true)
      ...
      logger.warn(confirmScriptsInstalledPretext(scriptsInstalled.length))
      logger.log("")
